...
class Solver(object):
    ...
    def _step(self):
        ...
        y_hat = model.forward(X_train)  # one less column cuz of bias, it will be added as ones
        dw = model.backward(y_hat)

        dloss = loss_func.backward(y_hat,y_train)
        dlossdw = dw*dloss
        dlossdw = dlossdw
        dlossdw = (np.sum(dlossdw,axis = 0)/float(X_train.shape[0])).reshape(2,1)
        opt.step(dlossdw)
